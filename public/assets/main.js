...
socket.on('game_start_response', (payload) => {
    ...
    let newNode = makeGameStartButton();
    $('.socket_' + payload.socket_id + ' button').replaceWith(newNode);
...
socket.on('join_room_response', (payload) => {
    ...
    */
    ...
    let newHTML = '<p class=\'join_room_response\'>' + payload.username + ' joined the chatroom ' + payload.room +
...
socket.on('player_disconnected', (payload) => {
    ...
    let newHTML = '<p class=\'left_room_response\'>' + payload.username + ' left the chat room. (There are'
    + payload.count + ' users in this room)</p>';
    let newNode = $(newHTML);
    newNode.hide();
    $('#messages').prepend(newNode);
    newNode.show("fade", 900);
...
socket.on('send_chat_message_response', (payload) => {
    ...
    let newNode = $(newHTML);
    newNode.hide();
    $('#messages').prepend(newNode); // Corrected variable name to newString
...
socket.on('game_update', (payload) => {
    ...
    if (my_color === 'white') {
        ...
        $("#my_color").html('<h3 id="my_color">Error: I dont\'t know what color I am</h3>');
    }
    ...
    if (payload.game.whose_turn === 'white') {
        ...
        $("#my_color").append('<h4 id="my_color">Error: I dont\'t know whose turn it is</h4>');
    }
    ...
    for (let row = 0; row < 8; row++) {
        for (let column = 0; column < 8; column++) {
            ...
            if (payload.game.whose_turn === my_color) {
                console.log(`It's my turn. My color: ${my_color}`);
                if (payload.game.legal_moves[row][column] === my_color.substring(0, 1)) {
                    console.log(`Legal move detected at row ${row}, column ${column}`);
...
$(() => {
    ...
    $("#quit").html("<a href='lobby.html?username=" + username + "' class='btn btn-danger' role='button'>Quit</a>"); // Corrected the missing quotation marks and added a missing closing angle bracket
